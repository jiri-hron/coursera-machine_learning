...
resp = eye(num_labels)(y,:)';

J = (-1/m) * ((resp .* log(A3)) + ((~resp) .*  ...
		log(ones(M = m, N = num_labels)' - A3)));
J = sum(sum(J)) + (lambda/(2*m)) * sum([Theta1(:,2:end)(:) ; ...
					 Theta2(:,2:end)(:)] .^ 2);

D3 = A3 - resp;
D2 = (Theta2' * D3) .* sigmoidGradient([ones(size(Z2,2),1)' ; Z2]);
